...
class Is_Loop(bpy.types.Operator):
	bl_idname = "node.is_loop"
	bl_label = "Is Loop"
	...
	def execute(self, context):
		...
		def is_loop_node_group():
			...
			#is_loop interface
			#Socket Selection
			selection_socket_1 = is_loop.interface.new_socket(name = "Selection", in_out='OUTPUT', socket_type = 'NodeSocketBool')
			selection_socket_1.attribute_domain = 'POINT'
			selection_socket_1.description = "Selected atoms form part of a loop, and not part of any secondary structure"
			
			#Socket Inverted
			inverted_socket_1 = is_loop.interface.new_socket(name = "Inverted", in_out='OUTPUT', socket_type = 'NodeSocketBool')
			inverted_socket_1.attribute_domain = 'POINT'
			
			#Socket And
			and_socket_1 = is_loop.interface.new_socket(name = "And", in_out='INPUT', socket_type = 'NodeSocketBool')
			and_socket_1.attribute_domain = 'POINT'
			and_socket_1.hide_value = True
			
			#Socket Or
			or_socket_1 = is_loop.interface.new_socket(name = "Or", in_out='INPUT', socket_type = 'NodeSocketBool')
			...
			#node Group Output
			group_output_1 = is_loop.nodes.new("NodeGroupOutput")
			group_output_1.name = "Group Output"
			group_output_1.is_active_output = True
			
			#node Group Input
			group_input_1 = is_loop.nodes.new("NodeGroupInput")
			group_input_1.name = "Group Input"
			
			#node MN_select_sec_struct.002
			mn_select_sec_struct_002 = is_loop.nodes.new("GeometryNodeGroup")
			...
			#Socket_1
			mn_select_sec_struct_002.inputs[2].default_value = 3
			...
			is_loop.links.new(mn_select_sec_struct_002.outputs[0], group_output_1.inputs[0])
			#group_input_1.And -> mn_select_sec_struct_002.And
			is_loop.links.new(group_input_1.outputs[0], mn_select_sec_struct_002.inputs[0])
			#group_input_1.Or -> mn_select_sec_struct_002.Or
			is_loop.links.new(group_input_1.outputs[1], mn_select_sec_struct_002.inputs[1])
			#mn_select_sec_struct_002.Inverted -> group_output_1.Inverted
			is_loop.links.new(mn_select_sec_struct_002.outputs[1], group_output_1.inputs[1])
			return is_loop
		...
		mod = obj.modifiers.new(name = "Is Loop", type = 'NODES')
		mod.node_group = is_loop
		return {'FINISHED'}

def menu_func(self, context):
	self.layout.operator(Is_Loop.bl_idname)
