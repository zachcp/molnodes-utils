...
class Style_Surface(bpy.types.Operator):
	bl_idname = "node.style_surface"
	bl_label = "Style Surface"
	...
	def execute(self, context):
		...
		def _mn_utils_style_surface_new_node_group():
			...
			triangulate_socket.default_value = True
		...
		def style_surface_node_group():
			...
			triangulate_socket_1.default_value = False
		...
		mod = obj.modifiers.new(name = "Style Surface", type = 'NODES')
		mod.node_group = style_surface
		return {'FINISHED'}

def menu_func(self, context):
	self.layout.operator(Style_Surface.bl_idname)
