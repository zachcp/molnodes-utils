...
class Is_Side_Chain(bpy.types.Operator):
	bl_idname = "node.is_side_chain"
	bl_label = "Is Side Chain"
	...
	def execute(self, context):
		...
		def is_side_chain_node_group():
			...
			#Socket Selection
			selection_socket = is_side_chain.interface.new_socket(name = "Selection", in_out='OUTPUT', socket_type = 'NodeSocketBool')
			selection_socket.attribute_domain = 'POINT'
			selection_socket.description = "True if atom is part of the side chain for either an amino acid or a nucleic acid"
			
			#Socket Inverted
			inverted_socket = is_side_chain.interface.new_socket(name = "Inverted", in_out='OUTPUT', socket_type = 'NodeSocketBool')
			inverted_socket.attribute_domain = 'POINT'
			
			#Socket And
			and_socket = is_side_chain.interface.new_socket(name = "And", in_out='INPUT', socket_type = 'NodeSocketBool')
			and_socket.attribute_domain = 'POINT'
			and_socket.hide_value = True
			
			#Socket Or
			or_socket = is_side_chain.interface.new_socket(name = "Or", in_out='INPUT', socket_type = 'NodeSocketBool')
			...
			#node Boolean Math.001
			boolean_math_001_2 = is_side_chain.nodes.new("FunctionNodeBooleanMath")
			boolean_math_001_2.name = "Boolean Math.001"
			boolean_math_001_2.operation = 'OR'
			
			#node Group Input
			group_input_5 = is_side_chain.nodes.new("NodeGroupInput")
			group_input_5.name = "Group Input"
			
			#node Boolean Math
			boolean_math_1 = is_side_chain.nodes.new("FunctionNodeBooleanMath")
			boolean_math_1.name = "Boolean Math"
			boolean_math_1.operation = 'AND'
			
			#node Group Output
			group_output_5 = is_side_chain.nodes.new("NodeGroupOutput")
			group_output_5.name = "Group Output"
			group_output_5.is_active_output = True
			
			#node Group.001
			group_001 = is_side_chain.nodes.new("GeometryNodeGroup")
			group_001.name = "Group.001"
			group_001.node_tree = _mn_select_nucleic
			
			#node Group.002
			group_002 = is_side_chain.nodes.new("GeometryNodeGroup")
			...
			#node Boolean Math.002
			boolean_math_002_2 = is_side_chain.nodes.new("FunctionNodeBooleanMath")
			...
			#Set locations
			boolean_math_001_2.location = (-460.0, -80.0)
			group_input_5.location = (-280.0, 20.0)
			boolean_math_1.location = (-120.0, 20.0)
			group_output_5.location = (240.00001525878906, 20.0)
			group_001.location = (-740.0, -80.0)
			group_002.location = (-740.0, -220.0)
			group_2.location = (-300.0, -80.0)
			boolean_math_002_2.location = (59.99999237060547, 20.0)
			...
			is_side_chain.links.new(group_input_5.outputs[0], boolean_math_1.inputs[0])
			#group_001.Is Side Chain -> boolean_math_001_2.Boolean
			is_side_chain.links.new(group_001.outputs[1], boolean_math_001_2.inputs[0])
			#group_002.Is Side Chain -> boolean_math_001_2.Boolean
			is_side_chain.links.new(group_002.outputs[1], boolean_math_001_2.inputs[1])
			#boolean_math_001_2.Boolean -> group_2.Fallback
			is_side_chain.links.new(boolean_math_001_2.outputs[0], group_2.inputs[1])
			#group_2.Boolean -> boolean_math_1.Boolean
			is_side_chain.links.new(group_2.outputs[0], boolean_math_1.inputs[1])
			#boolean_math_002_2.Boolean -> group_output_5.Selection
			is_side_chain.links.new(boolean_math_002_2.outputs[0], group_output_5.inputs[0])
			#boolean_math_1.Boolean -> boolean_math_002_2.Boolean
			is_side_chain.links.new(boolean_math_1.outputs[0], boolean_math_002_2.inputs[0])
			#group_input_5.Or -> boolean_math_002_2.Boolean
			is_side_chain.links.new(group_input_5.outputs[1], boolean_math_002_2.inputs[1])
			#boolean_math_002_2.Boolean -> boolean_math_003_2.Boolean
			is_side_chain.links.new(boolean_math_002_2.outputs[0], boolean_math_003_2.inputs[0])
			#boolean_math_003_2.Boolean -> group_output_5.Inverted
			is_side_chain.links.new(boolean_math_003_2.outputs[0], group_output_5.inputs[1])
		...
		mod = obj.modifiers.new(name = "Is Side Chain", type = 'NODES')
		mod.node_group = is_side_chain
		return {'FINISHED'}

def menu_func(self, context):
	self.layout.operator(Is_Side_Chain.bl_idname)
