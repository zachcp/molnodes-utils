...
class Is_Helix(bpy.types.Operator):
	bl_idname = "node.is_helix"
	bl_label = "Is Helix"
	...
	def execute(self, context):
		...
		def is_helix_node_group():
			...
			#node Group Output
			group_output_1 = is_helix.nodes.new("NodeGroupOutput")
			group_output_1.name = "Group Output"
			group_output_1.is_active_output = True
			
			#node Group Input
			group_input_1 = is_helix.nodes.new("NodeGroupInput")
			group_input_1.name = "Group Input"
			
			#node MN_select_sec_struct.002
			mn_select_sec_struct_002 = is_helix.nodes.new("GeometryNodeGroup")
			...
			#Socket_1
			mn_select_sec_struct_002.inputs[2].default_value = 1
			...
			is_helix.links.new(mn_select_sec_struct_002.outputs[0], group_output_1.inputs[0])
			#group_input_1.And -> mn_select_sec_struct_002.And
			is_helix.links.new(group_input_1.outputs[0], mn_select_sec_struct_002.inputs[0])
			#group_input_1.Or -> mn_select_sec_struct_002.Or
			is_helix.links.new(group_input_1.outputs[1], mn_select_sec_struct_002.inputs[1])
			#mn_select_sec_struct_002.Inverted -> group_output_1.Inverted
			is_helix.links.new(mn_select_sec_struct_002.outputs[1], group_output_1.inputs[1])
			return is_helix
		...
		mod = obj.modifiers.new(name = "Is Helix", type = 'NODES')
		mod.node_group = is_helix
		return {'FINISHED'}

def menu_func(self, context):
	self.layout.operator(Is_Helix.bl_idname)
