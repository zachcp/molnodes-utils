...
class Color_Chain_(bpy.types.Operator):
	bl_idname = "node.color_chain_"
	bl_label = "Color Chain_"
	...
	def execute(self, context):
		...
		def color_chain__node_group():
			color_chain_ = bpy.data.node_groups.new(type = 'GeometryNodeTree', name = "Color Chain_")
			...
			#Socket Color
			color_socket = color_chain_.interface.new_socket(name = "Color", in_out='OUTPUT', socket_type = 'NodeSocketColor')
			color_socket.default_value = (0.0, 0.0, 0.0, 0.0)
			color_socket.attribute_domain = 'POINT'
			color_socket.description = "The colors based on `chain_id`"
			
			#Socket Chain A
			chain_a_socket = color_chain_.interface.new_socket(name = "Chain A", in_out='INPUT', socket_type = 'NodeSocketColor')
			chain_a_socket.default_value = (0.4614596664905548, 0.6000000238418579, 0.23999999463558197, 1.0)
			chain_a_socket.attribute_domain = 'POINT'
			chain_a_socket.description = "Color to set for Chain A"
			
			#Socket Chain B
			chain_b_socket = color_chain_.interface.new_socket(name = "Chain B", in_out='INPUT', socket_type = 'NodeSocketColor')
			chain_b_socket.default_value = (0.23999999463558197, 0.6000000238418579, 0.5739687085151672, 1.0)
			chain_b_socket.attribute_domain = 'POINT'
			chain_b_socket.description = "Color to set for Chain B"
			
			#Socket ...
			____socket = color_chain_.interface.new_socket(name = "...", in_out='INPUT', socket_type = 'NodeSocketColor')
			...
			#node Group Output
			group_output_5 = color_chain_.nodes.new("NodeGroupOutput")
			group_output_5.name = "Group Output"
			group_output_5.is_active_output = True
			
			#node Switch.001
			switch_001 = color_chain_.nodes.new("GeometryNodeSwitch")
			...
			#node Switch.002
			switch_002 = color_chain_.nodes.new("GeometryNodeSwitch")
			switch_002.name = "Switch.002"
			switch_002.input_type = 'RGBA'
			
			#node Switch
			switch = color_chain_.nodes.new("GeometryNodeSwitch")
			switch.name = "Switch"
			...
			#node Group Input
			group_input_5 = color_chain_.nodes.new("NodeGroupInput")
			...
			color_chain_.links.new(group_059.outputs[0], switch.inputs[0])
			#group_input_5.Chain A -> switch.True
			color_chain_.links.new(group_input_5.outputs[0], switch.inputs[2])
			#switch_001.Output -> switch_002.False
			color_chain_.links.new(switch_001.outputs[0], switch_002.inputs[1])
			#group_059.Is Sheet -> switch_001.Switch
			color_chain_.links.new(group_059.outputs[1], switch_001.inputs[0])
			#group_059.Is Loop -> switch_002.Switch
			color_chain_.links.new(group_059.outputs[3], switch_002.inputs[0])
			#switch_002.Output -> group_output_5.Color
			color_chain_.links.new(switch_002.outputs[0], group_output_5.inputs[0])
			#switch.Output -> switch_001.False
			color_chain_.links.new(switch.outputs[0], switch_001.inputs[1])
			#group_input_5.Chain B -> switch_001.True
			color_chain_.links.new(group_input_5.outputs[1], switch_001.inputs[2])
			#group_input_5.... -> switch_002.True
			color_chain_.links.new(group_input_5.outputs[2], switch_002.inputs[2])
			return color_chain_
		...
		mod = obj.modifiers.new(name = "Color Chain_", type = 'NODES')
		mod.node_group = color_chain_
		return {'FINISHED'}

def menu_func(self, context):
	self.layout.operator(Color_Chain_.bl_idname)
