...
class Is_Solvent(bpy.types.Operator):
	bl_idname = "node.is_solvent"
	bl_label = "Is Solvent"
	...
	def execute(self, context):
		...
		def is_solvent_node_group():
			is_solvent = bpy.data.node_groups.new(type = 'GeometryNodeTree', name = "Is Solvent")
			...
			#Socket Selection
			selection_socket_1 = is_solvent.interface.new_socket(name = "Selection", in_out='OUTPUT', socket_type = 'NodeSocketBool')
			selection_socket_1.attribute_domain = 'POINT'
			selection_socket_1.description = "True if atoms is part of the solvent"
			
			#Socket Inverted
			inverted_socket_1 = is_solvent.interface.new_socket(name = "Inverted", in_out='OUTPUT', socket_type = 'NodeSocketBool')
			inverted_socket_1.attribute_domain = 'POINT'
			
			#Socket And
			and_socket_1 = is_solvent.interface.new_socket(name = "And", in_out='INPUT', socket_type = 'NodeSocketBool')
			and_socket_1.attribute_domain = 'POINT'
			and_socket_1.hide_value = True
			
			#Socket Or
			or_socket_1 = is_solvent.interface.new_socket(name = "Or", in_out='INPUT', socket_type = 'NodeSocketBool')
			...
			#node Group Output
			group_output_1 = is_solvent.nodes.new("NodeGroupOutput")
			group_output_1.name = "Group Output"
			group_output_1.is_active_output = True
			
			#node Group Input
			group_input_1 = is_solvent.nodes.new("NodeGroupInput")
			group_input_1.name = "Group Input"
			
			#node Group.014
			group_014 = is_solvent.nodes.new("GeometryNodeGroup")
			group_014.name = "Group.014"
			group_014.node_tree = _mn_select_attribute
			#Socket_2
			group_014.inputs[2].default_value = "is_solvent"
			...
			is_solvent.links.new(group_014.outputs[0], group_output_1.inputs[0])
			#group_input_1.And -> group_014.And
			is_solvent.links.new(group_input_1.outputs[0], group_014.inputs[0])
			#group_input_1.Or -> group_014.Or
			is_solvent.links.new(group_input_1.outputs[1], group_014.inputs[1])
			#group_014.Inverted -> group_output_1.Inverted
			is_solvent.links.new(group_014.outputs[1], group_output_1.inputs[1])
			return is_solvent
		...
		mod = obj.modifiers.new(name = "Is Solvent", type = 'NODES')
		mod.node_group = is_solvent
		return {'FINISHED'}

def menu_func(self, context):
	self.layout.operator(Is_Solvent.bl_idname)
