...
class Color_Rainbow(bpy.types.Operator):
	bl_idname = "node.color_rainbow"
	bl_label = "Color Rainbow"
	...
	def execute(self, context):
		...
		def chain_info_node_group():
			...
			per_chain_socket.default_value = True
		...
		def color_rainbow_node_group():
			color_rainbow = bpy.data.node_groups.new(type = 'GeometryNodeTree', name = "Color Rainbow")
			...
			#Socket Color
			color_socket = color_rainbow.interface.new_socket(name = "Color", in_out='OUTPUT', socket_type = 'NodeSocketColor')
			color_socket.default_value = (0.800000011920929, 0.800000011920929, 0.800000011920929, 1.0)
			color_socket.attribute_domain = 'POINT'
			
			#Socket Per Chain
			per_chain_socket_1 = color_rainbow.interface.new_socket(name = "Per Chain", in_out='INPUT', socket_type = 'NodeSocketBool')
			per_chain_socket_1.default_value = False
			per_chain_socket_1.attribute_domain = 'POINT'
			
			#Socket Offset
			offset_socket_1 = color_rainbow.interface.new_socket(name = "Offset", in_out='INPUT', socket_type = 'NodeSocketFloat')
			offset_socket_1.default_value = 0.0
			offset_socket_1.min_value = 0.0
			offset_socket_1.max_value = 1.0
			offset_socket_1.subtype = 'FACTOR'
			offset_socket_1.attribute_domain = 'POINT'
			offset_socket_1.description = "Offset rainbow along chain"
			
			#Panel Color
			color_panel = color_rainbow.interface.new_panel("Color")
			#Socket Color Saturation
			color_saturation_socket = color_rainbow.interface.new_socket(name = "Color Saturation", in_out='INPUT', socket_type = 'NodeSocketFloat', parent = color_panel)
			color_saturation_socket.default_value = 0.800000011920929
			...
			#Socket Color Value
			color_value_socket = color_rainbow.interface.new_socket(name = "Color Value", in_out='INPUT', socket_type = 'NodeSocketFloat', parent = color_panel)
			...
			#node Group Output
			group_output_4 = color_rainbow.nodes.new("NodeGroupOutput")
			group_output_4.name = "Group Output"
			group_output_4.is_active_output = True
			
			#node Group Input
			group_input_4 = color_rainbow.nodes.new("NodeGroupInput")
			group_input_4.name = "Group Input"
			
			#node Combine Color
			combine_color = color_rainbow.nodes.new("FunctionNodeCombineColor")
			...
			#node Group.001
			group_001_1 = color_rainbow.nodes.new("GeometryNodeGroup")
			group_001_1.name = "Group.001"
			group_001_1.node_tree = chain_info
			...
			group_001_1.outputs[6].hide = True
			
			#node Math
			math_1 = color_rainbow.nodes.new("ShaderNodeMath")
			math_1.name = "Math"
			math_1.operation = 'ADD'
			math_1.use_clamp = False
			
			#node Math.001
			math_001 = color_rainbow.nodes.new("ShaderNodeMath")
			...
			#Set locations
			group_output_4.location = (420.0, 220.0)
			group_input_4.location = (-440.0, 120.0)
			combine_color.location = (240.0, 220.0)
			group_001_1.location = (-260.0, 220.0)
			math_1.location = (-80.0, 220.0)
			math_001.location = (80.0, 220.0)
			...
			#combine_color.Color -> group_output_4.Color
			color_rainbow.links.new(combine_color.outputs[0], group_output_4.inputs[0])
			#group_input_4.Color Saturation -> combine_color.Green
			color_rainbow.links.new(group_input_4.outputs[2], combine_color.inputs[1])
			#group_input_4.Color Value -> combine_color.Blue
			color_rainbow.links.new(group_input_4.outputs[3], combine_color.inputs[2])
			#group_001_1.Factor -> math_1.Value
			color_rainbow.links.new(group_001_1.outputs[0], math_1.inputs[0])
			#math_1.Value -> math_001.Value
			color_rainbow.links.new(math_1.outputs[0], math_001.inputs[0])
			#math_001.Value -> combine_color.Red
			color_rainbow.links.new(math_001.outputs[0], combine_color.inputs[0])
			#group_input_4.Offset -> math_1.Value
			color_rainbow.links.new(group_input_4.outputs[1], math_1.inputs[1])
			#group_input_4.Per Chain -> group_001_1.Per Chain
			color_rainbow.links.new(group_input_4.outputs[0], group_001_1.inputs[0])
		...
		mod = obj.modifiers.new(name = "Color Rainbow", type = 'NODES')
		mod.node_group = color_rainbow
		return {'FINISHED'}

def menu_func(self, context):
	self.layout.operator(Color_Rainbow.bl_idname)
