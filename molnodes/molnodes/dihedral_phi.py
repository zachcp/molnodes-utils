...
class Dihedral_Phi(bpy.types.Operator):
	bl_idname = "node.dihedral_phi"
	bl_label = "Dihedral Phi"
	...
	def execute(self, context):
		...
		def dihedral_phi_node_group():
			dihedral_phi = bpy.data.node_groups.new(type = 'GeometryNodeTree', name = "Dihedral Phi")
			...
			#node Group Output
			group_output_12 = dihedral_phi.nodes.new("NodeGroupOutput")
			group_output_12.name = "Group Output"
			group_output_12.is_active_output = True
			
			#node Group.009
			group_009 = dihedral_phi.nodes.new("GeometryNodeGroup")
			group_009.name = "Group.009"
			group_009.node_tree = dihedral_angle
			
			#node Group
			group_3 = dihedral_phi.nodes.new("GeometryNodeGroup")
			group_3.name = "Group"
			...
			#node Group.001
			group_001_2 = dihedral_phi.nodes.new("GeometryNodeGroup")
			...
			#node Group.002
			group_002_2 = dihedral_phi.nodes.new("GeometryNodeGroup")
			...
			#node Group.003
			group_003_1 = dihedral_phi.nodes.new("GeometryNodeGroup")
			group_003_1.name = "Group.003"
			group_003_1.node_tree = backbone_position
			#Socket_3
			group_003_1.inputs[0].default_value = 0
			#Socket_2
			group_003_1.inputs[1].default_value = 'backbone_C'
			#Socket_4
			group_003_1.inputs[2].default_value = -1
			
			#node Group.004
			group_004 = dihedral_phi.nodes.new("GeometryNodeGroup")
			group_004.name = "Group.004"
			group_004.node_tree = fallback_float
			#Socket_0
			group_004.inputs[0].default_value = "Phi"
			
			
			
			
			#Set locations
			group_output_12.location = (880.0, 240.0)
			group_009.location = (119.99998474121094, 240.0)
			group_3.location = (-139.99998474121094, 40.0)
			group_001_2.location = (-139.99998474121094, 180.0)
			group_002_2.location = (-139.99998474121094, 320.0)
			group_003_1.location = (-139.99998474121094, -100.0)
			group_004.location = (480.0000305175781, 320.0)
			
			#Set dimensions
			group_output_12.width, group_output_12.height = 140.0, 100.0
			group_009.width, group_009.height = 299.8184509277344, 100.0
			group_3.width, group_3.height = 165.52227783203125, 100.0
			group_001_2.width, group_001_2.height = 165.52227783203125, 100.0
			group_002_2.width, group_002_2.height = 165.52227783203125, 100.0
			group_003_1.width, group_003_1.height = 165.52227783203125, 100.0
			group_004.width, group_004.height = 140.0, 100.0
			
			#initialize dihedral_phi links
			#group_009.BC -> group_output_12.BC
			dihedral_phi.links.new(group_009.outputs[3], group_output_12.inputs[3])
			#group_009.CD⟂(BC) -> group_output_12.CD⟂(BC)
			dihedral_phi.links.new(group_009.outputs[2], group_output_12.inputs[2])
			#group_3.Position -> group_009.C
			dihedral_phi.links.new(group_3.outputs[0], group_009.inputs[2])
			#group_001_2.Position -> group_009.B
			dihedral_phi.links.new(group_001_2.outputs[0], group_009.inputs[1])
			#group_003_1.Position -> group_009.D
			dihedral_phi.links.new(group_003_1.outputs[0], group_009.inputs[3])
			#group_002_2.Position -> group_009.A
			dihedral_phi.links.new(group_002_2.outputs[0], group_009.inputs[0])
			#group_009.BA⟂(BC) -> group_output_12.BA⟂(BC)
			dihedral_phi.links.new(group_009.outputs[1], group_output_12.inputs[1])
			#group_009.Angle -> group_004.Fallback
			dihedral_phi.links.new(group_009.outputs[0], group_004.inputs[1])
			#group_004.Value -> group_output_12.Phi
			dihedral_phi.links.new(group_004.outputs[0], group_output_12.inputs[0])
			return dihedral_phi
		...
		mod = obj.modifiers.new(name = "Dihedral Phi", type = 'NODES')
		mod.node_group = dihedral_phi
		return {'FINISHED'}

def menu_func(self, context):
	self.layout.operator(Dihedral_Phi.bl_idname)
