...
class Color_Sec_Struct(bpy.types.Operator):
	bl_idname = "node.color_sec_struct"
	bl_label = "Color Sec Struct"
	...
	def execute(self, context):
		...
		def color_sec_struct_node_group():
			color_sec_struct = bpy.data.node_groups.new(type = 'GeometryNodeTree', name = "Color Sec Struct")
			...
			#Socket Color
			color_socket = color_sec_struct.interface.new_socket(name = "Color", in_out='OUTPUT', socket_type = 'NodeSocketColor')
			color_socket.default_value = (0.0, 0.0, 0.0, 0.0)
			color_socket.attribute_domain = 'POINT'
			color_socket.description = "The colors based on secondary structure"
			
			#Socket Alpha Helix
			alpha_helix_socket = color_sec_struct.interface.new_socket(name = "Alpha Helix", in_out='INPUT', socket_type = 'NodeSocketColor')
			alpha_helix_socket.default_value = (0.16202937066555023, 0.623960554599762, 0.1946178376674652, 1.0)
			alpha_helix_socket.attribute_domain = 'POINT'
			alpha_helix_socket.description = "Color to set for alpha helices"
			
			#Socket Beta Sheet
			beta_sheet_socket = color_sec_struct.interface.new_socket(name = "Beta Sheet", in_out='INPUT', socket_type = 'NodeSocketColor')
			beta_sheet_socket.default_value = (1.0000001192092896, 0.14995980262756348, 0.14995980262756348, 1.0)
			beta_sheet_socket.attribute_domain = 'POINT'
			beta_sheet_socket.description = "Color to set for beta-sheets"
			
			#Socket Loop
			loop_socket = color_sec_struct.interface.new_socket(name = "Loop", in_out='INPUT', socket_type = 'NodeSocketColor')
			loop_socket.default_value = (0.17144113779067993, 0.3662526309490204, 0.7991029620170593, 1.0)
			...
			#node Group Output
			group_output_5 = color_sec_struct.nodes.new("NodeGroupOutput")
			group_output_5.name = "Group Output"
			group_output_5.is_active_output = True
			
			#node Switch.001
			switch_001 = color_sec_struct.nodes.new("GeometryNodeSwitch")
			switch_001.name = "Switch.001"
			switch_001.input_type = 'RGBA'
			
			#node Switch.002
			switch_002 = color_sec_struct.nodes.new("GeometryNodeSwitch")
			switch_002.name = "Switch.002"
			switch_002.input_type = 'RGBA'
			
			#node Switch
			switch = color_sec_struct.nodes.new("GeometryNodeSwitch")
			switch.name = "Switch"
			switch.input_type = 'RGBA'
			
			#node Group Input
			group_input_5 = color_sec_struct.nodes.new("NodeGroupInput")
			group_input_5.name = "Group Input"
			
			#node Named Attribute
			named_attribute = color_sec_struct.nodes.new("GeometryNodeInputNamedAttribute")
			...
			#node Group.059
			group_059 = color_sec_struct.nodes.new("GeometryNodeGroup")
			...
			#Set locations
			group_output_5.location = (278.192138671875, 0.0)
			switch_001.location = (80.0, -140.0)
			switch_002.location = (80.0, -300.0)
			switch.location = (80.0, 20.0)
			group_input_5.location = (-240.0, -200.0)
			named_attribute.location = (-240.0, -60.0)
			group_059.location = (-240.0, 120.0)
			...
			#group_059.Is Helix -> switch.Switch
			color_sec_struct.links.new(group_059.outputs[0], switch.inputs[0])
			#group_input_5.Alpha Helix -> switch.True
			color_sec_struct.links.new(group_input_5.outputs[0], switch.inputs[2])
			#switch_001.Output -> switch_002.False
			color_sec_struct.links.new(switch_001.outputs[0], switch_002.inputs[1])
			#group_059.Is Sheet -> switch_001.Switch
			color_sec_struct.links.new(group_059.outputs[1], switch_001.inputs[0])
			#group_059.Is Loop -> switch_002.Switch
			color_sec_struct.links.new(group_059.outputs[3], switch_002.inputs[0])
			#switch_002.Output -> group_output_5.Color
			color_sec_struct.links.new(switch_002.outputs[0], group_output_5.inputs[0])
			#switch.Output -> switch_001.False
			color_sec_struct.links.new(switch.outputs[0], switch_001.inputs[1])
			#group_input_5.Beta Sheet -> switch_001.True
			color_sec_struct.links.new(group_input_5.outputs[1], switch_001.inputs[2])
			#group_input_5.Loop -> switch_002.True
			color_sec_struct.links.new(group_input_5.outputs[2], switch_002.inputs[2])
			#named_attribute.Attribute -> switch.False
			color_sec_struct.links.new(named_attribute.outputs[0], switch.inputs[1])
		...
		mod = obj.modifiers.new(name = "Color Sec Struct", type = 'NODES')
		mod.node_group = color_sec_struct
		return {'FINISHED'}

def menu_func(self, context):
	self.layout.operator(Color_Sec_Struct.bl_idname)
